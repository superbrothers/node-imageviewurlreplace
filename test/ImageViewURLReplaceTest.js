...
module.exports = {
    ...
    , "複数の$EXTRACT": function (test) {
      ivur("http://www.vector.co.jp/magazine/softnews/120414/n1204141.html", function (url, referer, res) {
        test.equal(url, "http://www.vector.co.jp/magazine/softnews/120414/images/n12041411b.gif");
        test.done();
      });
    }
    , "リファラ付き": function (test) {
      ivur("http://hobby-c.rash.jp/igafioz/ref/1330986364242.htm", function (url, referer, res) {
  ...
  , "VIEWER Mode": {
    "正しく取得できる": function (test) {
      ivur("http://map.yahoo.co.jp/pl?type=scroll&lat=35.67832667&lon=139.77044378&sc=3&mode=map&pointer=on", function (url, referer, res) {
  ...
  , "DEFAULT Mode": {
    "正しく取得できる": function (test) {
      ivur("http://upload.wikimedia.org/wikipedia/en/f/fa/Google_Chrome_2011_computer_icon.svg", function (url, referer, res) {
  ...
  , "NG": {
    "正しくNG判定できている": function (test) {
      ivur("http://ipatukouta.altervista.org/test.jpg", function (url, referer, res) {
        test.equal(url, null);
        test.done();
      });
    }
